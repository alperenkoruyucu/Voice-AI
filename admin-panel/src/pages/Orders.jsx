...
export default function Orders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setError("Sipariş listesi yüklenirken bir hata oluştu.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 md:p-8 space-y-6 relative">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl shadow border border-gray-200">
          ...
          {(statusFilter || dateFilter) && (
            <button onClick={handleResetFilters} className="mt-2 text-blue-600 hover:underline text-sm font-semibold">Tüm siparişleri göster</button>
          )}
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage((prev) => Math.max(prev - 1, 1))} disabled={page === 1} className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium bg-white hover:bg-gray-50 text-gray-700 disabled:opacity-50 transition-colors">← Önceki</button>
              <button onClick={() => setPage((prev) => Math.min(prev + 1, meta.totalPages))} disabled={page >= meta.totalPages} className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium bg-white hover:bg-gray-50 text-gray-700 disabled:opacity-50 transition-colors">Sonraki →</button>
            </div>
          </div>
        </div>
      )}
            ...
            {orderDetail && !detailLoading && !detailError && (
              <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
                <span className="text-xs font-semibold text-gray-500">
                  {updatingStatus ? '🔄 Statü güncelleniyor...' : 'Süreç Aksiyonları:'}
                </span>
                ...
                <div className="flex flex-wrap gap-2 justify-end w-full sm:w-auto">
                  {orderDetail.status === 'RECEIVED' && (
                    <>
                      <button
                        ...
                        className="bg-amber-500 hover:bg-amber-600 text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors shadow disabled:opacity-50"
                      >
                        👨‍🍳 Hazırlanıyor'a Geçir
                      </button>
                      <button
                        onClick={() => handleUpdateStatus('CANCELLED')}
                        disabled={updatingStatus}
                        className="bg-red-100 hover:bg-red-200 text-red-700 font-bold px-3 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
                      >
                        ✕ İptal Et
                      </button>
                    </>
                  ...
                  {orderDetail.status === 'PREPARING' && (
                    <>
                      <button
                        ...
                        disabled={updatingStatus}
                        className="bg-purple-600 hover:bg-purple-700 text-white font-bold px-4 py-2 rounded-lg text-sm transition-colors shadow disabled:opacity-50"
                      >
                        🛵 Yola Çıktı / Dağıtımda
                      </button>
                      <button
                        onClick={() => handleUpdateStatus('CANCELLED')}
                        disabled={updatingStatus}
                  ...
                  {orderDetail.status === 'DELIVERING' && (
                    <button
                      ...
                      disabled={updatingStatus}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-2 rounded-lg text-sm transition-colors shadow disabled:opacity-50"
                    >
                  ...
                  {(orderDetail.status === 'COMPLETED' || orderDetail.status === 'CANCELLED') && (
                    <span className="text-xs font-bold italic text-gray-400 py-2">
                      Bu sipariş kapanmıştır, statü değiştirilemez.
                    </span>
                  )}

                  <button
                    onClick={handleCloseDetail}
                    className="px-4 py-2 border border-gray-300 hover:bg-gray-100 text-gray-700 font-semibold rounded-lg text-sm transition-colors ml-2"
                  >
                    Kapat
                  </button>
                </div>
